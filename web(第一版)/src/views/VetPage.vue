    ...
    <el-row :gutter="24">
      ...
      <el-col :span="10" v-if="userState.role === 'USER' || userState.role === 'ADMIN'">
        <el-card class="consultation-creation">
          <template #header><div class="card-header">发起在线咨询 (由执业药师接诊)</div></template>
          <el-form :model="consultationForm" label-position="top">
            ...
            <el-button type="primary" size="large" @click="createConsultation" class="full-width">
        ...
        <el-card class="consultation-history-card">
          ...
          <el-table :data="historyList" border stripe height="460" class="history-table">
            ...
            <el-table-column prop="diagnosis" label="诊断结果" min-width="200">
              <template #default="{ row }">
                <div v-if="row.diagnosis || row.prescriptionId" class="diagnosis-wrapper">
                  <el-tag type="success" effect="dark" class="diagnosis-tag">{{ row.diagnosis || '诊断已完成' }}</el-tag>
                </div>
            ...
            <el-table-column prop="prescriptionId" label="电子处方" width="100" align="center">
              <template #default="{ row }">
                <el-tag v-if="row.prescriptionId" type="danger" size="small" effect="dark" class="rx-id-tag clickable" @click="showPrescriptionDetails(row.prescriptionId)">
                  电子处方
                </el-tag>
                <span v-else>-</span>
              </template>
          ...
          <el-form :model="prescriptionForm" label-position="top">
            ...
            <div class="prescription-builder">
              ...
              <el-row :gutter="8" style="margin-top: 8px">
                ...
                <el-col :span="4"><el-button type="success" plain @click="appendDrug" class="full-width">添加</el-button></el-col>
              </el-row>
            </div>
            ...
            <el-button type="danger" size="large" @click="createPrescription" class="full-width" :disabled="!prescriptionForm.items.length">
              签署并下发电子处方
            </el-button>
          </el-form>
        </el-card>
    ...
    <el-dialog v-model="prescriptionDetailVisible" title="电子处方详情" width="500px">
      <div v-if="currentPrescription" class="prescription-detail-content">
        <div class="detail-header">
          ...
          <div class="detail-row"><span>临床诊断：</span><el-tag type="success">{{ currentPrescription.diagnosis }}</el-tag></div>
        </div>
...
.card-header { display: flex; align-items: center; gap: 8px; font-weight: 700; font-size: 16px; }
.log-display { background: #f8fafc; padding: 16px; border-radius: 6px; font-family: monospace; font-size: 13px; color: #475569; border: 1px solid #e2e8f0; margin-top: 10px; }
.prescription-builder { background: #f1f5f9; padding: 20px; border-radius: 8px; border: 1px dashed #cbd5e1; }
.builder-header { font-weight: 600; margin-bottom: 12px; font-size: 14px; color: var(--text-muted); }
.session-log-card, .audit-card { margin-top: 24px; }
.audit-controls { display: flex; gap: 12px; align-items: center; }
...
.detail-row span { color: #64748b; width: 90px; display: inline-block; }
.detail-row strong { color: #1e293b; }

.detail-footer { margin-top: 30px; text-align: center; }
.disclaimer { font-size: 13px; color: #f59e0b; margin-bottom: 20px; font-style: italic; }
</style>
